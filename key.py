...
def demo():
    ...
    signature = key.sign(data=my_post.encode())
    raw = struct.pack('<i', len(signature))+signature+my_post.encode()
    key2 = Key(public_key=key.public_key)
    signature_length, = struct.unpack_from('<i', raw, offset=0)
    mysignature = raw[4:signature_length+4]
    print('content:', raw[signature_length+4:].decode())
    print('verify signature with content:', 
            key2.verify(b58encode(mysignature), raw[signature_length+4:]))
